#!/usr/bin/env python3
"""
Run a suite of riscv-tests ELFs from a directory using the existing
Spike-vs-DUT comparator (`scripts/run_riscv_test_compare_spike.py`).
...
Outputs:
  - per-test artifacts in --outdir (hex, spike log/sig, dut log/sig)
...
def run_one(
    ...
    *,
    ...
    cmd = [
        "python3",
        "scripts/run_riscv_test_compare_spike.py",
        "--elf",
        str(elf),
        "--outdir",
        str(outdir),
    ...
    # Capture per-test runner output into a file for debugging.
...
def main() -> int:
    ...
    ap.add_argument("--build-verilator", action="store_true", help="Rebuild Verilator binary once before running")
    args = ap.parse_args()
    ...
    if args.build_verilator:
        subprocess.run(["make", "verilator"], check=True)
    ...
    print(f"[suite] dir={tests_dir} pattern={args.pattern} count={len(elfs)} outdir={outdir}")
    ...
    print(f"[suite] PASS={passed} FAIL={failed} summary={summary_path}")

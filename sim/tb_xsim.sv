        ...
        if (done) begin
            ...
            if (dut.wb_inst == 32'h3020_0073) begin
                ...
                if ((dut.wb_alu_out == tohost_addr) && (dut.wb_store_data != 32'h0)) begin
                    ...
                    $display("[tb_xsim] tohost write: addr=%08x data=%08x", tohost_addr, dut.wb_store_data);
                    $display("[tb_xsim] signature cfg: SIG=%0s BEGIN_SIG=%08x END_SIG=%08x", sigfile, begin_sig, end_sig);
                    dump_signature();
                    $finish;
                end
            end else begin

...
// XSim testbench for RISC-V CPU with Spike-like logging
// - Uses BRAM IP (blk_mem_gen_0) for instruction memory (loaded from COE file)
// - Produces Spike-compatible commit log for verification
// - Stops on first non-zero write to tohost
module tb_xsim;
    // ----------------------------
    // Clock / Reset
    // ----------------------------
    ...
    // For XSim, we use axi_uartlite_simple behavioral model.
    // ------------------------------------------------------------------------
    ...
    initial begin
        ...
        if (log_fd == 0) begin
            $display("[tb_xsim] ERROR: cannot open log file %0s", logfile);
            $finish;
        end
        $display("[tb_xsim] Writing Spike-like log to %0s", logfile);
    end
    ...
    task automatic dump_signature();
        ...
        fd = $fopen(sigfile, "w");
        if (fd == 0) begin
            $display("[tb_xsim] ERROR: cannot open signature file %0s", sigfile);
            return;
        end
    ...
    initial begin
        ...
        forever begin
            ...
            if (cycles >= max_cycles) begin
                $display("[tb_xsim] Reached MAX_CYCLES=%0d, stopping.", max_cycles);
                $finish;
            end
        end
    end
        ...
        if (done) begin
            ...
            if (dut.wb_inst == 32'h3020_0073) begin
                ...
                if ((dut.wb_alu_out == tohost_addr) && (dut.wb_store_data != 0)) begin
                    ...
                    $display("[tb_xsim] tohost write: addr=%016x data=%016x", tohost_addr, dut.wb_store_data);
                    $display("[tb_xsim] signature cfg: SIG=%0s BEGIN_SIG=%016x END_SIG=%016x", sigfile, begin_sig, end_sig);
                    dump_signature();
                    $finish;
                end
            end else begin
